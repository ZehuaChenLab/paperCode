...
class SwinASPP(nn.Module):
    def __init__(self, input_size, input_dim, out_dim, cross_attn,
        ...
        if input_size == 24:
            ...
            self.possible_window_sizes = [i for i in range(start_window_size, input_size//2 + 1) if input_size % i == 0]
        ...
        self.proj2 = nn.Linear(len(self.layers) * input_dim, out_dim)
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            features.append(out)
        # print(len(features))
        feature_2_3 = torch.cat([features[1], features[2]], dim=-1)
        feature_2_3 = self.proj1(feature_2_3)

        feature_1_4 = torch.cat([features[0], features[3], feature_2_3], dim=-1)
        feature_1_4 = torch.cat([x, feature_1_4], dim=-1)
        feature_1_4 = self.proj2(feature_1_4)
        ...
        if self.norm is not None:
            features = self.norm(feature_1_4)
        if self.activation is not None:
            features = self.activation(feature_1_4)
